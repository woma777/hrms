...
public class LanguageService {
    ...
    public List<LanguageResponse> getAllLanguages(UUID tenantId,
        ...
        TenantDto tenant = validationUtil.getTenantById(tenantId);
        Applicant applicant = validationUtil.getApplicantById(tenant.getId(), applicantId);
        List<Language> languages = languageRepository.findByTenantIdAndApplicant(tenant.getId(), applicant);
        return languages.stream().map(languageMapper::mapToDto).toList();
    }
