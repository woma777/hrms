...
public class ExamResultService {
    ...
    public ExamResultResponse createExamResult(UUID tenantId,
        ...
        Recruitment recruitment = validationUtil
                .getRecruitmentById(tenant.getId(), recruitmentId);
        Applicant applicant = validationUtil
                .getApplicantById(tenant.getId(), applicantId);
        boolean examResultExists = examResultRepository
                .existsByTenantIdAndApplicant(tenant.getId(), applicant);
        if (examResultExists) {
            throw new ResourceExistsException(
                    "Exam Result already exists for the specified applicant");
        }
        ExamResult examResult = examResultMapper
                .mapToEntity(tenant, recruitment, applicant, request);
    ...
    public List<ExamResultResponse> getAllExamResult(UUID tenantId,
        ...
        return examResults.stream().map(examResultMapper::mapToDto).toList();
    }
    ...
    private ExamResult getExamResultById(UUID tenantId,
