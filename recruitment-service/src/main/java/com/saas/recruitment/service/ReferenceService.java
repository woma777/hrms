...
public class ReferenceService {
    ...
    public List<ReferenceResponse> getAllReferences(UUID tenantId,
        ...
        TenantDto tenant = validationUtil.getTenantById(tenantId);
        Applicant applicant = validationUtil.getApplicantById(tenant.getId(), applicantId);
        List<Reference> references = referenceRepository.findByTenantIdAndApplicant(tenant.getId(), applicant);
        return references.stream().map(referenceMapper::mapToDto).toList();
    }
