...
public class ExamResultMapper {
    ...
    public ExamResult mapToEntity(
            TenantDto tenant, Recruitment recruitment, Applicant applicant, ExamResultRequest request) {
        ...
        if (request.getWrittenExam() <= assessmentWeight.getWrittenExam()) {
            ...
            throw new IllegalArgumentException(
                    "Written exam score must be less than or equal to " + assessmentWeight.getWrittenExam());
        }
        if (request.getInterview() <= assessmentWeight.getInterview()) {
            examResult.setInterview(request.getInterview());
        } else {
            throw new IllegalArgumentException(
                    "Interview score must be less than or equal to " + assessmentWeight.getInterview());
        }
        if (request.getCGPA() <= assessmentWeight.getCGPA()) {
            examResult.setCGPA(request.getCGPA());
        } else {
            throw new IllegalArgumentException(
                    "CGPA score must be less than or equal to " + assessmentWeight.getCGPA());
        }
        if (request.getExperience() <= assessmentWeight.getExperience()) {
            examResult.setExperience(request.getExperience());
        } else {
            throw new IllegalArgumentException(
                    "Experience score must be less than or equal to " + assessmentWeight.getExperience());
        }
        if (request.getPracticalExam() <= assessmentWeight.getPracticalExam()) {
            examResult.setPracticalExam(request.getPracticalExam());
        } else {
            throw new IllegalArgumentException(
                    "Practical exam score must be less than or equal to " + assessmentWeight.getPracticalExam());
        }
        if (request.getOther() <= assessmentWeight.getOther()) {
            examResult.setOther(request.getOther());
        } else {
            throw new IllegalArgumentException(
                    "Other score must be less than or equal to " + assessmentWeight.getOther());
        }

        examResult.setTotal(request.getWrittenExam()
    ...
    public ExamResult updateExamResult(
            TenantDto tenant, Recruitment recruitment, ExamResult examResult, ExamResultRequest request) {
        ...
        if (request.getWrittenExam() != null && request.getWrittenExam() <= assessmentWeight.getWrittenExam()) {
            examResult.setWrittenExam(request.getWrittenExam());
        } else {
            throw new IllegalArgumentException(
                    "Written exam score must be less than or equal to " + assessmentWeight.getWrittenExam());
        }
        if (request.getInterview() != null && request.getInterview() <= assessmentWeight.getInterview()) {
            examResult.setInterview(request.getInterview());
        } else {
            throw new IllegalArgumentException(
                    "Interview score must be less than or equal to " + assessmentWeight.getInterview());
        }
        if (request.getCGPA() != null && request.getCGPA() <= assessmentWeight.getCGPA()) {
            examResult.setCGPA(request.getCGPA());
        } else {
            throw new IllegalArgumentException(
                    "CGPA score must be less than or equal to " + assessmentWeight.getCGPA());
        }
        if (request.getExperience() != null && request.getExperience() <= assessmentWeight.getExperience()) {
            examResult.setExperience(request.getExperience());
        } else {
            throw new IllegalArgumentException(
                    "Experience score must be less than or equal to " + assessmentWeight.getExperience());
        }
        if (request.getPracticalExam() != null && request.getPracticalExam() <= assessmentWeight.getPracticalExam()) {
            examResult.setPracticalExam(request.getPracticalExam());
        } else {
            throw new IllegalArgumentException(
                    "Practical exam score must be less than or equal to " + assessmentWeight.getPracticalExam());
        }
        if (request.getOther() != null && request.getOther() <= assessmentWeight.getOther()) {
            examResult.setOther(request.getOther());
        } else {
            throw new IllegalArgumentException(
                    "Other score must be less than or equal to " + assessmentWeight.getOther());
