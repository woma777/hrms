...
public class ExperienceRequest {
    ...
    @NotNull(message = "Location ID cannot be null") private UUID locationId;

    @NotBlank(message = "Job title cannot be blank")
    @Size(min = 2, max = 100, message = "Job title must be between 2 and 100 characters")
    private String jobTitle;

    @NotNull(message = "Salary cannot be null") @DecimalMin(value = "0.0", message = "Salary must be greater than or equal to 0")
    ...
    @NotNull(message = "Start date cannot be null") @Past(message = "Start date must be a past date")
