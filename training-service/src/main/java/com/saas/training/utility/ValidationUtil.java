...
public class ValidationUtil {
    ...
    public EmployeeDto getEmployeeByName(UUID tenantId, String firstName, String middleName, String lastName) {
        ...
        try {
            return employeeServiceClient.getEmployeeByName(tenantId, firstName, middleName, lastName);
        } catch (FeignException.NotFound e) {
    ...
    public CountryDto getCountryById(UUID tenantId, UUID countryId) {
    ...
    public FieldOfStudyDto getFieldOfStudyById(UUID tenantId, UUID fieldOfStudyId) {
        ...
        try {
            ...
            throw new ResourceNotFoundException("Field of study not found with id '" + fieldOfStudyId + "'");
        }
    }
    ...
    public TrainingInstitution getInstitutionById(UUID tenantId, UUID institutionId) {
        ...
        return trainingInstitutionRepository
    ...
    public TrainingCourseCategory getCategoryById(UUID tenantId, UUID categoryId) {
        ...
        return trainingCourseCategoryRepository
                .findById(categoryId)
                .filter(category -> category.getTenantId().equals(tenantId))
                .orElseThrow(
                        () -> new ResourceNotFoundException("Course category not found with id '" + categoryId + "'"));
    }
    ...
    public TrainingCourse getTrainingCourseById(UUID tenantId, UUID courseId) {
        ...
        return trainingCourseRepository
                ...
                .orElseThrow(
                        () -> new ResourceNotFoundException("Training course not found with id '" + courseId + "'"));
    }

    public CheckedDocument getCheckedDocumentById(UUID tenantId, UUID documentId) {

        return checkedDocumentRepository
                .findById(documentId)
    ...
    public PreServiceTrainee getTraineeById(UUID tenantId, UUID traineeId) {
        ...
        return preServiceTraineeRepository
                .findById(traineeId)
                .filter(trainee -> trainee.getTenantId().equals(tenantId))
                .orElseThrow(() ->
                        new ResourceNotFoundException("Pre Service Trainee not found with id '" + traineeId + "'"));
    }

    public InternshipStudent getStudentById(UUID tenantId, UUID studentId) {

        return internshipStudentRepository
                .findById(studentId)
                .filter(intern -> intern.getTenantId().equals(tenantId))
                .orElseThrow(() ->
                        new ResourceNotFoundException("Internship student not found with id '" + studentId + "'"));
    }
    ...
    public University getUniversityById(UUID tenantId, UUID universityId) {
        ...
        return universityRepository
                ...
                .orElseThrow(
                        () -> new ResourceNotFoundException("University not found with id '" + universityId + "'"));
    }

    public PreServiceCourseType getCourseTypeById(UUID tenantId, UUID courseTypeId) {

        return preServiceCourseTypeRepository
                .findById(courseTypeId)
                .filter(type -> type.getTenantId().equals(tenantId))
                .orElseThrow(
                        () -> new ResourceNotFoundException("Course type not found with id '" + courseTypeId + "'"));
    }

    public PreServiceCourse getPreServiceCourseById(UUID tenantId, UUID courseId) {

        return preServiceCourseRepository
                .findById(courseId)
                .filter(course -> course.getTenantId().equals(tenantId))
                .orElseThrow(
                        () -> new ResourceNotFoundException("Pre Service Course Not Found with id '" + courseId + "'"));
    }
