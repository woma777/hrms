...
public class PreServiceTraineeResultService {
    ...
    public PreServiceTraineeResultResponse addTraineeResult(UUID tenantId,
        ...
        List<PreServiceCourse> traineeCourses =
                courseRepository.findByTenantIdAndPreServiceTraineesId(tenant.getId(), traineeId).stream()
        ...
        if (traineeResultRepository
                .existsByTenantIdAndPreServiceTraineeIdAndPreServiceCourseIdAndSemester(
                tenant.getId(), trainee.getId(), course.getId(),
                Semester.valueOf(request.getSemester().toUpperCase()))) {
            throw new ResourceExistsException(
                    "The result for trainee with id '" + trainee.getId() + "' and course with id '"
                            + course.getId() + "' in the '" + request.getSemester() +
                            "' semester already exists.");
        }
        PreServiceTraineeResult traineeResult = traineeResultMapper
                .mapToEntity(tenant, trainee, course, request);
    ...
    public PreServiceTraineeResult getResultById(UUID tenantId,
                                                 UUID resultId) {
