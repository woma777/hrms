...
public class TenantMapper {
    ...
    public Tenant updateTenant(Tenant tenant, TenantRequest tenantRequest, MultipartFile file) throws IOException {
        if (tenantRequest.getTenantName() != null) tenant.setTenantName(tenantRequest.getTenantName());
        if (tenantRequest.getAbbreviatedName() != null) tenant.setAbbreviatedName(tenantRequest.getAbbreviatedName());
        if (tenantRequest.getEstablishedYear() != null) tenant.setEstablishedYear(tenantRequest.getEstablishedYear());
        if (tenantRequest.getMission() != null) tenant.setMission(tenantRequest.getMission());
        if (tenantRequest.getVision() != null) tenant.setVision(tenantRequest.getVision());
