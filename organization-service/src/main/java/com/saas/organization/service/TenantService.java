...
public class TenantService {
    ...
    public TenantResponse createTenant(TenantRequest tenantRequest, MultipartFile file) throws IOException {
        ...
        if (tenantRepository.existsByTenantName(tenantRequest.getTenantName())) {
            throw new ResourceExistsException("Tenant with Name " + tenantRequest.getTenantName() + " already exists");
        }
    ...
    public List<TenantResponse> getAllTenants() {
        ...
        return tenants.stream().map(tenantMapper::mapToDto).collect(Collectors.toList());
    }
    ...
    public TenantResponse updateTenant(UUID id, TenantRequest tenantRequest, MultipartFile file) throws IOException {
        ...
        Tenant tenant = tenantRepository
                .findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Tenant not found with id: " + id + " "));
        if (tenantRepository.existsByTenantNameAndIdNot(tenantRequest.getTenantName(), tenant.getId())) {
            throw new ResourceExistsException("Tenant with Name " + tenantRequest.getTenantName() + " already exists");
        }
        if (tenantRepository.existsByAbbreviatedNameAndIdNot(tenantRequest.getAbbreviatedName(), tenant.getId())) {
            throw new ResourceExistsException(
                    "Tenant with Abbreviated name " + tenantRequest.getTenantName() + " already exists");
        }
