...
public class DepartmentService {
    ...
    public DepartmentResponse createDepartment(UUID tenantId, DepartmentRequest departmentRequest) {
        ...
        if (departmentRepository.existsByDepartmentNameAndTenantId(
            ...
            throw new ResourceExistsException(
                    "Department with Name " + departmentRequest.getDepartmentName() + " already exists");
        }
    ...
    public DepartmentResponse addSubDepartment(UUID parentId, UUID tenantId, DepartmentRequest subDepartmentRequest) {
        ...
        Department parentDepartment = departmentRepository
                ...
                .filter(dept -> dept.getTenant().getId().equals(tenant.getId()))
                .orElseThrow(() -> new ResourceNotFoundException(
                        "Parent department not found with id: " + parentId + " for the specified tenant"));

        if (departmentRepository.existsByDepartmentNameAndParentDepartment(
                subDepartmentRequest.getDepartmentName(), parentDepartment)) {
            throw new ResourceExistsException("Sub-department with Name " + subDepartmentRequest.getDepartmentName()
                    + " already exists under parent department with id: "
                    + parentId);
        }
    ...
    public void removeSubDepartment(UUID parentId, UUID tenantId, UUID subId) {
        ...
        Department subDepartment = departmentRepository
                .findById(subId)
                .filter(dept -> dept.getTenant().getId().equals(tenant.getId()))
                .filter(dept -> dept.getParentDepartment() != null
                        && dept.getParentDepartment().getId().equals(parentId))
                .orElseThrow(() -> new ResourceNotFoundException(
                        "Sub-department not found with id: " + subId + " for the specified parent department"));
    ...
    public List<DepartmentResponse> getAllDepartmentsWithoutChildren() {
        ...
        return departments.stream().map(departmentMapper::mapToDto).collect(Collectors.toList());
    }
    ...
    public List<Department> getAllChildDepartments(UUID departmentId, UUID tenantId) {
        ...
        Department department = departmentRepository
                ...
                .orElseThrow(() -> new ResourceNotFoundException(
                        "Department not found with id: " + departmentId + " for the specified tenant"));
    ...
    public List<DepartmentResponse> getAllParentDepartments(UUID tenantId) {
        ...
        return parentDepartments.stream().map(departmentMapper::mapToDto).collect(Collectors.toList());
    }

    public DepartmentResponse transferChildDepartment(
            UUID tenantId, UUID childDepartmentId, UUID newParentDepartmentId) {

        // Fetch the departments from the database
        Department childDepartment = departmentRepository
                .findById(childDepartmentId)
                .orElseThrow(() -> new ResourceNotFoundException("Child department not found"));
        Department newParentDepartment = departmentRepository
                .findById(newParentDepartmentId)
    ...
    @Transactional
    public DepartmentResponse transferParentDepartment(
            UUID tenantId, UUID parentDepartmentId, UUID newParentDepartmentId) {

        // Fetch the departments from the database
        Department parentDepartment = departmentRepository
                .findById(parentDepartmentId)
