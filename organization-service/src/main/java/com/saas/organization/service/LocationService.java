...
public class LocationService {
    ...
    public LocationResponse createLocation(UUID tenantId, LocationRequest locationRequest) {
        ...
        LocationType locationType = locationTypeRepository
                .findById(locationRequest.getLocationTypeId())
                .orElseThrow(() -> new ResourceNotFoundException(
                        "Location type not found with id: " + locationRequest.getLocationTypeId()));
    ...
    public LocationResponse getLocationById(UUID id, UUID tenantId) {
        ...
        Location location = locationRepository
                ...
                .filter(loc -> loc.getTenant().getId().equals(tenant.getId()))
                .orElseThrow(() -> new ResourceNotFoundException(
                        "Location not found with id: " + id + " for the specified tenant"));

        return locationMapper.mapToDto(location);
    }

    public LocationResponse updateLocation(UUID id, UUID tenantId, LocationRequest locationRequest) {
    ...
    public LocationResponse addSubLocation(UUID parentId, UUID tenantId, LocationRequest subLocationRequest) {
    ...
    public void removeSubLocation(UUID parentId, UUID tenantId, UUID subId) {
        ...
        Location parentLocation = locationRepository
                ...
                .filter(loc -> loc.getTenant().getId().equals(tenant.getId()))
                .orElseThrow(() -> new ResourceNotFoundException(
                        "Parent location not found with id: " + parentId + " for the specified tenant"));

        Location subLocation = locationRepository
                .findById(subId)
                .filter(loc -> loc.getTenant().getId().equals(tenant.getId()))
                .filter(loc -> loc.getParentLocation() != null
                        && loc.getParentLocation().getId().equals(parentId))
                .orElseThrow(() -> new ResourceNotFoundException(
                        "Sub-location not found with id: " + subId + " for the specified parent location"));
    ...
    public List<Location> getAllChildLocations(UUID parentId, UUID tenantId) {
        ...
        Location department =
                locationRepository.findById(parentId).orElseThrow(() -> new RuntimeException("Location is not found"));
    ...
    public List<LocationResponse> getAllParentLocations(UUID tenantId) {
        ...
        return parentLocations.stream().map(locationMapper::mapToDto).collect(Collectors.toList());
    }
}
