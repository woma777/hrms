...
public class PayGradeService {
    ...
    public PayGradeResponse createPayGrade(UUID tenantId, PayGradeRequest payGradeRequest) {
        ...
        if (payGradeRepository.existsBySalaryStepAndTenantId(payGradeRequest.getSalaryStep(), tenant.getId())) {
            throw new ResourceExistsException(
                    "Pay Grade with Name " + payGradeRequest.getSalaryStep() + " already exists");
        }
    ...
    public PayGradeResponse getPayGradeById(UUID id, UUID tenantId) {
        ...
        PayGrade payGrade = payGradeRepository
                .findById(id)
                .filter(pay -> pay.getTenant().getId().equals(tenant.getId()))
    ...
    public List<PayGradeResponse> getPayGradesByJobGradeId(UUID jobGradeId, UUID tenantId) {
        ...
        List<PayGrade> payGrades = payGradeRepository.findByJobGradeId(jobGradeId).stream()
                .filter(pay -> pay.getTenant().getId().equals(tenant.getId()))
                .toList();

        return payGrades.stream().map(payGradeMapper::mapToDto).collect(Collectors.toList());
    }
