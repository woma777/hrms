...
public class JobRegistrationService {
    ...
    public JobRegistrationResponse registerJob(UUID tenantId, JobRegistrationRequest jobRegistrationRequest) {
        ...
        if (jobRegistrationRepository.existsByJobTitleAndTenantId(
            ...
            throw new ResourceExistsException(
                    "Job with Title " + jobRegistrationRequest.getJobTitle() + " already exists");
        }

        if (jobRegistrationRepository.existsByJobCode(jobRegistrationRequest.getJobCode())) {
            throw new ResourceExistsException(
                    "Job with Code " + jobRegistrationRequest.getJobCode() + " already exists");
        }

        JobRegistration jobRegistration = jobRegistrationMapper.mapToEntity(jobRegistrationRequest);
    ...
    public List<JobRegistrationResponse> getAllJobsByDepartment(UUID tenantId, UUID departmentId) {
        ...
        List<JobRegistration> jobRegistrations =
                jobRegistrationRepository.findByTenantAndDepartment(tenant, department);

        return jobRegistrations.stream().map(jobRegistrationMapper::mapToDto).collect(Collectors.toList());
    }
    ...
    public JobRegistrationResponse getJobById(UUID id, UUID tenantId) {
        ...
        JobRegistration jobRegistration = jobRegistrationRepository
                .findById(id)
                .filter(job -> job.getTenant().getId().equals(tenant.getId()))
                .orElseThrow(() ->
                        new ResourceNotFoundException("Job not found with id: " + id + " for the specified tenant"));
