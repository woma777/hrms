...
public class LocationTypeController {
    ...
    public ResponseEntity<?> createLocationType(
            @PathVariable("tenant-id") UUID tenantId, @RequestBody LocationTypeRequest locationTypeRequest) {

        permissionEvaluator.addLocationTypePermission(tenantId);

        LocationTypeResponse location = locationTypeService.createLocationType(tenantId, locationTypeRequest);
        return new ResponseEntity<>(location, HttpStatus.CREATED);
    }

    @GetMapping("/get-all")
    public ResponseEntity<?> getAllLocationTypes(@PathVariable("tenant-id") UUID tenantId) {
    ...
    @GetMapping("/get/{id}")
    public ResponseEntity<?> getLocationTypeById(@PathVariable UUID id, @PathVariable("tenant-id") UUID tenantId) {
    ...
    public ResponseEntity<?> updateLocationType(
        ...
        LocationTypeResponse location = locationTypeService.updateLocationType(id, tenantId, locationTypeRequest);
        return ResponseEntity.ok(location);
    }

    @DeleteMapping("/delete-locationType/{id}")
    public ResponseEntity<?> deleteLocationType(@PathVariable UUID id, @PathVariable("tenant-id") UUID tenantId) {
