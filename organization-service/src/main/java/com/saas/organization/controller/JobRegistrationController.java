...
public class JobRegistrationController {
    ...
    public ResponseEntity<?> registerJob(
            @PathVariable("tenant-id") UUID tenantId, @RequestBody JobRegistrationRequest jobRegistrationRequest) {

        permissionEvaluator.addJobPermission(tenantId);

        JobRegistrationResponse job = jobRegistrationService.registerJob(tenantId, jobRegistrationRequest);
        return new ResponseEntity<>(job, HttpStatus.CREATED);
    }

    @GetMapping("/get-all")
    public ResponseEntity<?> getAllJobs(@PathVariable("tenant-id") UUID tenantId) {
    ...
    public ResponseEntity<?> getAllJobsByTenantAndDepartment(
            @PathVariable("tenant-id") UUID tenantId, @PathVariable("departmentId") UUID departmentId) {
    ...
    @GetMapping("/get/{id}")
    public ResponseEntity<?> getJobById(@PathVariable UUID id, @PathVariable("tenant-id") UUID tenantId) {
    ...
    public ResponseEntity<?> updateJob(
        ...
        JobRegistrationResponse job = jobRegistrationService.updateJobs(id, tenantId, jobRegistrationRequest);
        return ResponseEntity.ok(job);
    }

    @DeleteMapping("/delete-job/{id}")
    public ResponseEntity<?> deleteJob(@PathVariable UUID id, @PathVariable("tenant-id") UUID tenantId) {
