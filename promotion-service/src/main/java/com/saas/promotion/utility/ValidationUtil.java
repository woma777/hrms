...
public class ValidationUtil {
    ...
    public RecruitmentDto getRecruitmentById(UUID tenantId, UUID recruitmentId) {
    ...
    public RecruitmentDto getRecruitmentByVacancyNumber(UUID tenantId, String vacancyNumber) {
        ...
        List<RecruitmentDto> recruitments = recruitmentServiceClient.getRecruitmentsByMode(tenantId, recruitmentMode);
    ...
    public Candidate getCandidateById(UUID tenantId, UUID promotionId) {

        return candidateRepository
                .findById(promotionId)
                .filter(ap -> ap.getTenantId().equals(tenantId))
                .orElseThrow(
                        () -> new ResourceNotFoundException("Apply promotion not found with id '" + promotionId + "'"));
    }

    public CriteriaName getCriteriaNameById(UUID tenantId, UUID criteriaNameId) {

        return criteriaNameRepository
                .findById(criteriaNameId)
                .filter(cn -> cn.getTenantId().equals(tenantId))
                .orElseThrow(() -> new ResourceNotFoundException(
                        "Promotion criteria name not found with id '" + criteriaNameId + "'"));
    }

    public PromotionCriteria getPromotionCriteria(UUID tenantId, UUID promotionCriteriaId) {

        return promotionCriteriaRepository
                .findById(promotionCriteriaId)
