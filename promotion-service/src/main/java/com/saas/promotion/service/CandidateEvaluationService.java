...
public class CandidateEvaluationService {
    ...
    public CandidateEvaluationResponse addCandidateEvaluation(UUID tenantId,
        ...
        Candidate candidate = validationUtil.getCandidateById(tenant.getId(), candidateId);
    ...
    public List<CandidateEvaluationResponse> getAllCandidateEvaluations(UUID tenantId,
        ...
        TenantDto tenant = validationUtil.getTenantById(tenantId);
        Candidate candidate = validationUtil.getCandidateById(tenantId, candidateId);
        List<CandidateEvaluation> evaluations =
                evaluationRepository.findByTenantIdAndCandidateId(tenant.getId(), candidate.getId());
        return evaluations.stream().map(evaluationMapper::mapToDto).toList();
    }
    ...
    public CandidateEvaluationResponse updateCandidateEvaluation(
            UUID tenantId, UUID candidateId, UUID evaluationId, double result) {

        TenantDto tenant = validationUtil.getTenantById(tenantId);
        Candidate candidate = validationUtil.getCandidateById(tenantId, candidateId);
        CandidateEvaluation evaluation = getCandidateEvaluation(tenant, candidate, evaluationId);
        PromotionCriteria criteria = validationUtil.getPromotionCriteria(
                tenant.getId(), evaluation.getPromotionCriteria().getId());
    ...
    public void validateResult(PromotionCriteria promotionCriteria,
                               double result) {
