...
public class PromoteCandidateMapper {
    ...
    public PromoteCandidate mapToEntity(TenantDto tenant, PromoteCandidateRequest request) {

        Candidate candidate = validationUtil.getCandidateById(tenant.getId(), request.getCandidateId());
        RecruitmentDto recruitment = validationUtil.getRecruitmentById(tenant.getId(), candidate.getRecruitmentId());
        PayGradeDto payGrade =
                validationUtil.getPayGradeById(tenant.getId(), recruitment.getJobId(), request.getPayGradeId());
    ...
    public PromoteCandidateResponse mapToDto(PromoteCandidate promoteCandidate) {
        ...
        response.setCandidate(candidateMapper.mapToDto(promoteCandidate.getCandidate()));
