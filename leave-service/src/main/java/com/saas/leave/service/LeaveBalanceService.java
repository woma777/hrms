...
public class LeaveBalanceService {
    ...
    public LeaveBalanceResponse calculateLeaveBalanceForEmployee(UUID tenantId, UUID employeeId, UUID budgetYearId) {
        ...
        logger.info(
                "Calculating leave balance for employee ID: {}, Budget Year ID: {}, Tenant ID: {}",
                employeeId,
                budgetYearId,
                tenantId);

        // Retrieve the budget year
        BudgetYear budgetYear = budgetYearRepository
                .findById(budgetYearId)
        ...
        int maxDays = leaveSettings.stream()
                ...
                .orElseThrow(
                        () -> new ResourceNotFoundException("Leave setting must be set for tenant ID: " + tenantId));
        ...
        return new LeaveBalanceResponse(
                employeeId, employee.getId(), budgetYear.getId(), totalLeaveDays, remainingLeaveDays, tenantId);
    }
    ...
    private void saveOrUpdateLeaveBalance(
            UUID employeeId, int remainingLeaveDays, int totalLeaveDays, UUID tenantId, UUID budgetYearId) {
        // Fetch existing leave balances for the employee and budget year
        List<LeaveBalance> leaveBalances =
                leaveBalanceRepository.findByEmployeeIdAndBudgetYearId(employeeId, budgetYearId);

        if (leaveBalances.isEmpty()) {
            // Create a new leave balance if none exists
            LeaveBalance newLeaveBalance =
                    createNewLeaveBalance(employeeId, totalLeaveDays, remainingLeaveDays, tenantId, budgetYearId);
    ...
    private LeaveBalance createNewLeaveBalance(
            UUID employeeId, int totalLeaveDays, int remainingLeaveDays, UUID tenantId, UUID budgetYearId) {
    ...
    @Transactional
    public void saveCurrentLeaveBalanceToHistory(
            UUID employeeId, int totalLeaveDays, int remainingLeaveDays, UUID budgetYearId, UUID tenantId) {
        BudgetYear budgetYear = budgetYearRepository
                .findById(budgetYearId)
                .orElseThrow(() -> new ResourceNotFoundException("Budget year not found for ID: " + budgetYearId));
