...
public class LeaveBalanceMapper {
    ...
    public LeaveBalance toEntity(LeaveBalanceRequest requestDTO) {
        ...
        if (requestDTO.getBudgetYearId() != null) {
            BudgetYear budgetYear = budgetYearRepository
                    .findById(requestDTO.getBudgetYearId())
                    .orElseThrow(() ->
                            new RuntimeException("BudgetYear not found with ID: " + requestDTO.getBudgetYearId()));
    ...
    public LeaveBalanceResponse mapToDto(LeaveBalance entity) {
        ...
        response.setBudgetYearId(
                entity.getBudgetYear() != null
                        ? entity.getBudgetYear().getId()
                        : null); // Ensure mapping of budgetYearId
