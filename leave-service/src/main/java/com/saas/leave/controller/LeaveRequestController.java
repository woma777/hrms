...
@RestController
@RequestMapping("/api/leave/leave-requests/{tenantId}")
...
public class LeaveRequestController {
    ...
    public ResponseEntity<LeaveRequestResponse> createLeaveRequest(
            @PathVariable("tenantId") UUID tenantId, @RequestBody LeaveRequestRequest leaveRequestRequest) {

        permissionEvaluator.addLeaveRequestPermission(tenantId);

        LeaveRequestResponse createdLeaveRequest =
                leaveRequestService.createLeaveRequest(tenantId, leaveRequestRequest);
    ...
    public ResponseEntity<LeaveRequestResponse> getLeaveRequestById(
            @PathVariable("tenantId") UUID tenantId, @PathVariable UUID id) {
    ...
    public ResponseEntity<LeaveRequestResponse> updateLeaveRequest(
        ...
        LeaveRequestResponse updatedLeaveRequest =
                leaveRequestService.updateLeaveRequest(tenantId, id, leaveRequestRequest);
    ...
    public ResponseEntity<LocalDate> calculateReturnDate(
            @RequestBody LeaveRequestRequest leaveRequestRequest, @PathVariable String tenantId) {
    ...
    public ResponseEntity<?> departmentApprove(
        ...
        LeaveRequestResponse response = leaveRequestService.approveLeaveRequest(tenantId, requestId, request, null);
        return ResponseEntity.ok(response);
    }

    @PutMapping("/hr-approve/{requestId}")
    public ResponseEntity<?> HrApprove(
            @PathVariable("tenantId") UUID tenantId, @PathVariable UUID requestId, @RequestParam String decision) {

        permissionEvaluator.hrApproveLeaveRequestPermission(tenantId);

        LeaveRequestResponse response = leaveRequestService.approveLeaveRequest(tenantId, requestId, null, decision);
        return ResponseEntity.ok(response);
    }

    @GetMapping("/get/status")
    public ResponseEntity<?> getLeaveRequestsByStatus(
            @PathVariable("tenantId") UUID tenantId, @RequestParam String status) {

        permissionEvaluator.getLeaveRequestsByStatusPermission(tenantId);

        List<LeaveRequestResponse> responses = leaveRequestService.getLeaveRequestsByStatus(tenantId, status);
