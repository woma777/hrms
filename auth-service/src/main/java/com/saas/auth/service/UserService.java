...
public class UserService {
    ...
    public UserResponse addUser(UUID tenantId,
        ...
        userRepresentation =
                usersResource.searchByUsername(employee.getEmployeeId(), true).getFirst();
    ...
    public UserResponse addAdminUser(UUID tenantId) {
        ...
        if (!existsByUsername.isEmpty()) {
            throw new ResourceExistsException("User with username '" + username + "' already exists.");
        }
        List<UserRepresentation> existsByEmail = usersResource
        ...
        return userRepresentations.stream()
                .filter(user -> {
                    Optional<String> userTenantId = Optional.ofNullable(user.getAttributes())
                            .map(attr -> attr.get("tenantId").getFirst());
                    return userTenantId.isPresent()
                            && userTenantId.get().equals(tenant.getId().toString());
                })
    ...
    public List<UserResponse> getUsersByRoleName(UUID tenantId,
        ...
        List<UserRepresentation> userRepresentations = usersResource.list().stream()
                .filter(user -> usersResource.get(user.getId()).roles().realmLevel().listAll().stream()
    ...
    public UserResponse getUserById(UUID tenantId,
                                    String userId) {
    ...
    public UserResponse getUserByUsername(UUID tenantId, String username) {
        ...
        userRepresentation = validationUtil.getUserRepresentation(tenant.getId(), userRepresentation.getId());
        return userMapper.mapToDto(userRepresentation);
    }

    public List<RoleResponse> getUserRoles(UUID tenantId, String userId) {
    ...
    public UserResponse updateUser(UUID tenantId,
        ...
        TenantDto tenant = validationUtil.getTenantById(tenantId);
        UsersResource usersResource = getUsersResource();
        UserRepresentation userRepresentation = validationUtil
                .getUserRepresentation(tenant.getId(), userId);
    ...
    public UserResponse updateUserEmail(UUID tenantId,
        ...
        List<UserRepresentation> existsByEmail = usersResource.searchByEmail(email, true);
        if (!existsByEmail.isEmpty()
                && !userRepresentation.getId().equals(existsByEmail.getFirst().getId())) {
            throw new ResourceExistsException(
                    "User with email " + userRepresentation.getEmail() + " already exists");
