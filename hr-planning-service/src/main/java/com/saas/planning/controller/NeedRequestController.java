...
@RestController
@RequestMapping("/api/hr-planning/need-requests/{tenantId}")
...
public class NeedRequestController {
    ...
    public ResponseEntity<NeedRequestResponseDto> createNeedRequest(
            @PathVariable("tenantId") UUID tenantId, @Valid @RequestBody NeedRequestRequestDto needRequestRequest) {
    ...
    @GetMapping("/get-all")
    public ResponseEntity<List<NeedRequestResponseDto>> getAllNeedRequests(@PathVariable("tenantId") UUID tenantId) {
    ...
    public ResponseEntity<NeedRequestResponseDto> getNeedRequestById(
            @PathVariable("tenantId") UUID tenantId, @PathVariable("id") UUID id) {
    ...
    public ResponseEntity<NeedRequestResponseDto> updateNeedRequest(
        ...
        NeedRequestResponseDto updatedNeedRequest =
                needRequestService.updateNeedRequest(tenantId, id, needRequestRequest);
    ...
    public ResponseEntity<List<NeedRequestResponseDto>> getNeedRequestsByStaffPlanId(
            @PathVariable("tenantId") UUID tenantId, @RequestParam("staffPlanId") UUID staffPlanId) {

        permissionEvaluator.getHrNeedRequestByStaffPlanIdPermission(tenantId);

        try {
            List<NeedRequestResponseDto> needRequests =
                    needRequestService.getNeedRequestByStaffplanId(tenantId, staffPlanId);
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(null); // Return HTTP 404 Not Found if resources are missing
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(null); // Return HTTP 500 for other server errors
        }
    }
