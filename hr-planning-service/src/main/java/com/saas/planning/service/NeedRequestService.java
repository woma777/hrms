...
public class NeedRequestService {
    ...
    public NeedRequestResponseDto createNeedRequest(UUID tenantId, NeedRequestRequestDto needRequestRequest) {
        ...
        DepartmentDto department =
                organizationServiceClient.getDepartmentById(needRequestRequest.getDepartmentId(), tenantId);
        StaffPlanDto jobRegistration =
                organizationServiceClient.getStaffPlanById(needRequestRequest.getStaffPlanId(), tenantId);
        TenantDto tenant = organizationServiceClient.getTenantById(tenantId);
        if (tenantId == null) {
            throw new IllegalArgumentException("TenantId must not be null");
        }

        if (needRequestRequest == null
                || needRequestRequest.getBudgetYearId() == null
    ...
    public List<NeedRequestResponseDto> getAllNeedRequests(UUID tenantId) {
        ...
        return hrNeedRequests.stream().map(needRequestMapper::mapToDto).collect(Collectors.toList());
    }
    ...
    public NeedRequestResponseDto getNeedRequestById(UUID tenantId, UUID id) {
        ...
        HrNeedRequest hrNeedRequest = needRequestRepository
                .findById(id)
